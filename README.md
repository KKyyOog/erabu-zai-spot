...
マイページではプロフィールのキャッシュ期限（10分）と本人確認の有効期限を分離し、
有効なLINE認証セッション（既定12時間）があればLIFFの初期化・LINEへの再認証を省略します。
ページを開いても認証期限は延長しません。通知設定は保存済みの友だち状態から取得します。
プロフィール取得後は画面とタブを先に表示し、投稿・問い合わせ履歴は各欄で読み込みます。
通知から指定された問い合わせへの移動は履歴の取得後に行います。

## 通知再送・送信制限の運用

申込み、状態変更、連絡先共有は、業務データと同じトランザクションで
`notification_outbox` に通知を保存します。画面処理では初回送信を試み、
失敗した通知は再送待ちで保持します。本番では以下を**5分ごと（`*/5 * * * *`）に実行するジョブ**に登録してください。
...
3. アプリを反映し、`python -m flask --app run send-notifications` を5分ごと、`python -m flask --app run clean-upload-jobs` を毎日実行するジョブを登録します。アプリと同じDB・LINE・Cloudinary設定を使用します。
4. 管理画面 `/admin/` で、通知の滞留時間・失敗理由・再送ジョブの最終完了を確認します。最終完了が15分以上前の場合は警告が表示されます。外部の自動アラート連携は含まれません。

## 運用の軽量化

既存の再送ジョブのスケジュールを `*/5 * * * *` に変更してください。
コードの反映だけでは外部ジョブのスケジュールは変わりません。
初回通知は引き続き画面処理で即時送信を試みます。失敗した通知の再送は、
再試行可能になってから通常最大約5分待ちます（滞留・障害時を除く）。
1回100件の上限は維持するため、滞留が続く場合は実行間隔を戻してください。

正常完了の記録は `operations` の1件を更新し、空実行でも監視時刻を更新します。
毎日の `clean-upload-jobs` は画像回収に続いて運用データも清掃します。
追加の定期ジョブは不要です。運用データのみの清掃は次で実行できます。

```bash
python -m flask --app run clean-operational-data
```

- 送信済み通知：作成時刻と最終試行後の時刻がともに30日より古いものを削除します。
- 運用ログ：30日より古い旧ジョブ完了記録と、対応する通知が残っていない通知エラーを削除します。
- 期限切れの送信制限カウンターを削除します。
- 各分類は1回最大1,000件です。既存の大量の履歴は複数回に分けて減ります。
- 未送信・失敗通知、最新のジョブ完了記録、通報、管理操作・マッチング状態の履歴、投稿は削除しません。

設定値は `NOTIFICATION_JOB_STALE_SECONDS=900`、
`SENT_NOTIFICATION_RETENTION_DAYS=30`、`OPERATION_LOG_RETENTION_DAYS=30` です。
保存日数を0以下にすると、その分類の保存期限による削除を無効にします。
本番反映前にDBバックアップと日次ジョブの稼働を確認してください。
テーブル・列の追加はありません。

通常の本番インストールは `requirements.txt` を使います。
旧Google Sheetsからの移行時だけ `pip install -r requirements-migration.txt` を実行します。
サーバー・DBの地域、ワーカー数と接続数は本番の配置・メモリ・待ち時間を確認して調整してください。
実測なしで同時処理数を減らす設定変更は行っていません。
