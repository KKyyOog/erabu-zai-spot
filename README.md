...
写真付き投稿では、大きなJPEG写真を送信前に長辺1600pxまで縮小します。
変換後の容量が小さくなる場合だけ使用し、PNG・GIF・WebPや変換非対応の環境では元の写真を送ります。
サーバーからの写真アップロードは最大2枚ずつ並行し、選択した順番を保ちます。
保存成功時は今回のアップロード記録のみを処理します。失敗時の後始末と定期清掃は引き続き有効です。
...
認可コードなどの機密値は出力せず、許可した診断項目のみを保存します。
自由なエラー本文やURLは保存しません。無効時はブラウザの診断送信も停止します。
ログは最大10件ずつ集約し、送信できなかった件数は次の送信で`diagnostics.dropped`として記録します。
LINE検証の一時障害は503として扱い、ログアウトせず画面から再試行できます。
詳しくは[ログイン調査メモ](docs/login-debug-review.md)を参照してください。
...
制限カウンターはDBで共有します。`TRUSTED_PROXY_HOPS` で指定した段数だけ
`X-Forwarded-For` の右側を信頼し、Flaskの `remote_addr` に反映します。
Render（`RENDER=true`）では既定で1段、その他では0段（転送ヘッダーを信用しない）です。
独自のプロキシ構成では実際の信頼できる段数に合わせて明示設定してください。
認証と診断ログの制限枠は別です。429応答では残り待機秒数を返し、認証画面に表示します。
